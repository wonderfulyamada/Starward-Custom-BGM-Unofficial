...
def process_frame(frame, timestamp, cfg, detector, state, show_debug):
    burst = detector.burst_metrics(
        frame,
        calibrate=state.state in (state.BATTLE, state.AWAKENING),
    ...
    if state.state in (state.BATTLE, state.AWAKENING):
        victory = detector.victory_score(frame)
        defeat = detector.defeat_score(frame)
    else:
        victory = defeat = 0.0

    scores = DetectorScores(go=go, victory=victory, defeat=defeat)
    state.update(timestamp, scores, burst)
    ...
    if show_debug:
        ...
        cv2.putText(
            debug,
            f"burst={burst.classification} emblem={burst.emblem_score:.3f} ticks={burst.normal_tick_edge:.3f}",
            (20, 70),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.65,
            (255, 255, 255),
